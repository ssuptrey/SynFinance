...
def create_sample_charts(data):
    ...
    ax1.set_xlabel('Transaction Amount (Rs.)', fontsize=12)
...
def generate_findings(data, metrics):
    ...
    if high_value_count > len(data) * 0.1:
        findings.append({
            ...
            'description': f"{high_value_count} transactions exceed Rs.10,000",
            'severity': 'info',
...
def demo_html_reports(data, metrics, charts, output_dir):
    ...
    print(f"   [OK] Generated: executive_summary.html ({len(html_exec):,} characters)")
    ...
    print(f"   [OK] Generated: technical_analysis.html ({len(html_tech):,} characters)")
    ...
    print(f"   [OK] Generated: fraud_detection.html ({len(html_fraud):,} characters)")
    ...
    print(f"   [OK] Generated: data_quality.html ({len(html_quality):,} characters)")
    
    print(f"\n[DIR] All HTML reports saved to: {output_dir}/")
...
def demo_excel_dashboards(data, output_dir):
    ...
    print(f"   [OK] Generated: transaction_dashboard.xlsx ({file_size:.1f} KB)")
    print(f"   [DATA] Sheets: Summary Dashboard, Transaction Data, Statistical Analysis,")
    ...
    print(f"   [OK] Generated: transaction_dashboard_no_fraud.xlsx ({file_size_no_fraud:.1f} KB)")
    
    print(f"\n[DIR] All Excel dashboards saved to: {output_dir}/")
...
def demo_pdf_export(output_dir):
    ...
    if PDF_AVAILABLE:
        try:
            ...
            print(f"   [OK] Generated: executive_summary.pdf ({file_size:.1f} KB)")
...
def print_summary(data, metrics, output_dir):
    ...
    print(f"   • Total Amount: Rs.{metrics['total_amount']:,.2f}")
    print(f"   • Average Amount: Rs.{metrics['avg_amount']:,.2f}")
...
def main():
    ...
    print(f"   [OK] Generated {len(data):,} transactions")
    
    # Calculate metrics
    print("\n[*] Calculating metrics...")
    metrics = calculate_metrics(data)
    print(f"   [OK] Calculated {len(metrics)} metrics")
